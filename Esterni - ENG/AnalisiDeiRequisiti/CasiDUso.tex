...
\end{itemize}
%TODO Grafico UC3 + Discussione inclusione
...
\begin{itemize}
	...
	\begin{enumerate}
		\item Ricerca corso di laurea [UC8]
	\end{enumerate}
	\item \textbf{Precondizione:} L'utente ha selezionato la registrazione come studente e ll sistema offre un campo per la selezione del corso di laurea e rimane in attesa dell'input;
...
\end{itemize}
\subsection{UC8 - Ricerca corso di laurea}
\begin{itemize}
	\item \textbf{Attori:} Utente non autenticato, Studente, Professore, Amministratore;
	\item \textbf{Scopo e Descrizione} L'utente ricerca un corso di laurea e se esiste il sistema lo mostra;
	\item \textbf {Scenario principale:}
	\begin{enumerate}
		\item Il sistema mostra un campo per inserire parole chiave;
		\item L'utente inserisce le parole chiave per ricercare un corso;
		\item Il sistema cerca nel modello di dati persistente dei corsi adeguati ai parametri di ricerca.
		\item Il sistema mostra la lista (anche vuota) dei corsi trovati.
	\end{enumerate}
	\item \textbf{Precondizione:} L'utente ha richiesto la ricerca di un corso di laurea;
	\item \textbf{Postcondizione:} Il sistema ha mostrato i corsi individuati nella sua ricerca.
\end{itemize}
...
\begin{figure}[H]
	...
	\label{fig:UC7.1 - Visualizzazione delle informazione degli esami ai quali è iscritto}
\end{figure}
