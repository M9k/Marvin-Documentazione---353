...
\begin{document}
	...
	Ogni documento formale deve passare gli stadi di "Sviluppo", "\citGloss{verifica}" e "Approvato".
	\begin{itemize}
	...
	\begin{itemize}
		...
		\item Dare il comando "git \citGloss{pull}". Nel caso in cui si verifichino dei conflitti:
		\begin{itemize}
			\item Dare il comando "git stash" per accantonare momentaneamente	le modifiche apportate;
			\item Dare il comando "git \citGloss{pull}" per ottenere ed applicare i \citGloss{commit} mancanti;
		...
		\item Dare il comando "git \citGloss{commit}" e successivamente riassumere le modifiche effettuate, in caso sia utile si può aggiungere un messaggio esteso di descrizione;
		\item Dare il comando "git \citGloss{push}" per completare l'operazione e fornire le modifiche agli altri membri del gruppo.
	\end{itemize}
