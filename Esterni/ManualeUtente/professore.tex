\documentclass[ManualeUtente]{subfiles}

\begin{document}

\chapter{Teacher}
\section{Account request}
\subsection{Request a new teacher account}
\begin{enumerate}
	\item For register as a teacher user click on the \textquotedblleft Register" link on the menu bar or on the home page;
	\begin{figure}[H]
		\centering
		\fcolorbox{black}{white}{\includegraphics[width=0.8\linewidth]{"image/Register"}}
		\caption{Click on "Register"}
		\label{fig:Click on "Register"}
	\end{figure} \newpage
	\item Insert your name, surname and select "Teacher" as role. Then click on \textquotedblleft SUBMIT" button;
	\begin{figure}[H]
		\centering
		\fcolorbox{black}{white}{\includegraphics[width=0.8\linewidth]{"image/RegisterP"}}
		\caption{Fill the fields}
		\label{fig:Fill the fields}
	\end{figure}
	
\end{enumerate}
\newpage
\section{Exams management}
\subsection{Register a student's grade}
\begin{enumerate}
	\item For assign a grade to students, go to the exams page by clicking on the \textquotedblleft Exams" link on the menu bar or \textquotedblleft Exams list" link on the homepage;
	\begin{figure}[H]
		\centering
		\fcolorbox{black}{white}{\includegraphics[width=0.8\linewidth]{"image/TeacherExam1"}}
		\caption{Click on "Exams"}
		\label{fig:Click on "Exams"}
	\end{figure}
	\item Click \textquotedblleft VIEW STUDENTS" related to the desired course.
	\begin{figure}[H]
		\centering
		\fcolorbox{black}{white}{\includegraphics[width=0.8\linewidth]{"image/TeacherExam2"}}
		\caption{Click on "VIEW STUDENTS"}
		\label{fig:Click on "VIEW STUDENTS"}
	\end{figure} \newpage
	\item Click on \textquotedblleft SET VOTE" button related to the desired student.
	\begin{figure}[H]
		\centering
		\fcolorbox{black}{white}{\includegraphics[width=0.8\linewidth]{"image/TeacherExam3"}}
		\caption{Click "SET VOTE"}
		\label{fig:Click "SET VOTE"}
	\end{figure}
	\item Then insert a grade from 18 to 30(for 30L write 31) and then click on \textquotedblleft SUBMIT" button to confirm.
	\begin{figure}[H]
		\centering
		\fcolorbox{black}{white}{\includegraphics[width=0.8\linewidth]{"image/TeacherExam4"}}
		\caption{Insert a grade}
		\label{fig:Insert a grade}
	\end{figure}
\end{enumerate}

\end{document}