...
\begin{document}
	...
	%TODO come gestire riferimenti  
	%TODO footnote messi come riferimenti informativi
	...
	\section{Qualità}
	\subsection{Descrizione}%TODO
	Questa sezione descrive le classificazioni, procedure per il calcolo e metriche aggiuntive.
	
	\subsection{Classificazione delle metriche}
	Per garantire la qualità del lavoro gli Amministratori hanno definito delle metriche, riportandole all'interno del Piano di qualifica, che devono però rispettare la seguente notazione \textbf{M[categ][macrocateg][num]}	dove:
	\begin{itemize}
		...
		\end{itemize}
		\item \textbf{macrocateg:} indica la macrocategoria della metrica, se esiste altrimenti non compare, può assumere i valori:
		\begin{itemize}
			\item \textbf{D:} per indicare i documenti;
			\item \textbf{S:} per indicare i software.
		\end{itemize}	
		\item \textbf{num:} indica il codice univoco della metrica come numero intero a tre cifre incrementale a partire da 1.
	\end{itemize}	
	
	\subsection{Metriche aggiuntive interne}
	\begin{itemize}
		\item \textbf{Metriche per i feedback di miglioramento:} i feedback permettono il miglioramento dello svolgimento dei differenti ruoli all'interno del progetto. 
	\end{itemize}
	...
	\begin{itemize}
		...
		\item \textbf{Documenti:} Per il controllo dei documenti prodotti utilizzeremo le funzionalità di Texstudio assieme a script bash eseguiti automaticamente da \citGloss{Travis} per controllare che non ci siano errori nei file LaTex, per assicurarsi che il Glossario non presenti mancanze o duplicati e verificare l'assenza di errori ortografici;
		...
		\item \textbf{Gestione tasklist:} Todo353swe\footnote{\nURI{todo353swe.netlify.com}} semplice sito per gestione tasklist realizzato dai membri del gruppo per utilizzo interno.
		\item \textbf{Analisi dinamica locale:} è stato individuato un plugin per Webstorm SonarLint\footnote{\nURI{https://plugins.jetbrains.com/plugin/7973-sonarlint}} come possibile soluzione iniziale da verificare, per risolvere tutte le necessità di analisi del codice locale. Maggiori dettagli sarannò introdotti durante la creazione dei Proof of concept per la Technology \citGloss{baseline}.
		\item \textbf{Analisi dinamica remota:} è stato individuato Sonarqube come possibile soluzione iniziale da accertare, per risolvere tutte le necessità di analisi del codice del gruppo. Maggiori dettagli sarannò introdotti durante la creazione dei Proof of concept per la Technology \citGloss{baseline}.
	\end{itemize}
