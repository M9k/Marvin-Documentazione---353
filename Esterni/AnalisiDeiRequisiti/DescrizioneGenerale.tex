...
\begin{itemize}
	\item La possibilità per gli studenti di iscriversi a un \citGloss{corso di laurea} e gestire il proprio percorso accademico;
...
\end{itemize}
Data la particolarità della rete \citGloss{Ethereum}, in cui ogni operazione di scrittura ha un costo economico, l'applicativo fornisce all'utente, in maniera preventiva, la possibilità di visualizzare la stima del prezzo di ogni operazione da effettuare.
...
L'utente, per accedere al sistema, deve possedere alcune nozioni base sul funzionamento della rete \citGloss{Ethereum} e del plugin \citGloss{Metamask}, conoscenze che possono essere acquisite tramite una breve guida fornita dall'applicazione.

\section{Piattaforma d'esecuzione}
Il prodotto finale può essere suddiviso in due macro parti con due diverse piattaforme d'esecuzione: il back-end è ospitato dalla rete \citGloss{Ethereum}, l'interfacciamento a esso (front-end) è fruibile tramite un qualsiasi \citGloss{browser} web predisposto all'installazione del plugin \citGloss{Metamask}.

\section{Vincoli generali}
L'utente, per usufruire del servizio, deve possedere un \citGloss{browser} con installato il plugin \citGloss{Metamask}, una connessione a internet e una coppia di chiavi (pubblica-privata) compatibile con la rete \citGloss{Ethereum}.\\
La coppia di chiavi viene fornita automaticamente alla accettazione della licenza di \citGloss{Metamask}, ma se l'utente desidera può utilizzare chiavi già in suo possesso o generate utilizzando altre procedure.	
